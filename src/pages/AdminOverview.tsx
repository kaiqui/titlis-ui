import { Activity, AlertOctagon, AlertTriangle, BarChart3, Bot, CheckCircle2, GitMerge, ShieldAlert, Users } from 'lucide-react'
...
export function AdminOverview() {
  ...
  const { compliance, remediations, pillars, users, ai } = overview
      ...
      <div className="space-y-6 px-4 py-6 lg:px-8">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <Card>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-3">
                ...
                <StatMini
                  label="Sem acesso"
                  value={fmt(users.neverAccessed)}
                  colorClass={users.neverAccessed > 0 ? 'text-amber-500' : undefined}
                />
              </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Bot size={15} /> Uso da ARIA
              </CardTitle>
            </CardHeader>
            {!ai.isConfigured ? (
              <p className="text-sm" style={{ color: 'var(--color-muted-foreground)' }}>
                ARIA não está configurada para este tenant.
              </p>
            ) : (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <StatMini label="Tokens usados" value={formatTokens(ai.tokensUsedMonth)} />
                  <StatMini label="Budget mensal" value={ai.monthlyTokenBudget ? formatTokens(ai.monthlyTokenBudget) : '∞'} />
                </div>
                {ai.monthlyTokenBudget != null && ai.monthlyTokenBudget > 0 && (
                  <div>
                    <div className="mb-1.5 flex justify-between text-xs" style={{ color: 'var(--color-muted-foreground)' }}>
                      <span>Consumo do mês</span>
                      <span className="font-bold">{(ai.usagePercent ?? 0).toFixed(0)}%</span>
                    </div>
                    <div className="h-2 overflow-hidden rounded-full" style={{ backgroundColor: 'var(--color-border)' }}>
                      <div
                        className={cn('h-full rounded-full', scoreBgColor(100 - (ai.usagePercent ?? 0)))}
                        style={{ width: `${Math.min(ai.usagePercent ?? 0, 100)}%` }}
                      />
                    </div>
                  </div>
                )}
        ...
        {/* Bloco 5 — Tabela de usuários */}
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Users size={15} /> Equipe na Plataforma
            </CardTitle>
          </CardHeader>
          {userList.length === 0 ? (
            <p className="text-sm" style={{ color: 'var(--color-muted-foreground)' }}>
