...
export function Recommendations() {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <Header title="Remediação" subtitle="Fila de workloads com findings abertos — clique para ver detalhes e ações." />
      ...
      <div className="flex-1 space-y-5 px-4 py-6 lg:px-8">
        ...
        <div className="flex justify-end">
          <ButtonDefault visual="ghost" label="Atualizar" icon={RefreshCw} onClick={() => void refetch()} />
        </div>
                  ...
                  <DetailPanel title="Próximas ações" subtitle="O essencial para tirar o item da fila.">
                    <InlineAccordion title="Checklist" defaultOpen>
                      <div className="space-y-2">
                        {[
                          ...
                          selected.githubPrUrl ? 'Acompanhar PR até rollout.' : 'Usar o assistente ARIA para abrir PR de remediação.',
                        ].map(item => (
                          <div key={item} className="rounded-2xl px-3 py-2" style={{ backgroundColor: 'var(--color-muted)' }}>
                            <p className="text-sm" style={{ color: 'var(--color-foreground)' }}>{item}</p>
                          </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <ButtonDefault
                        ...
                        icon={ArrowRight}
                        onClick={() => navigate(`/scorecards/${selected.id}`)}
                      />
                      <ButtonDefault
