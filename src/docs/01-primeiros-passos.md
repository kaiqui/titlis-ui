...
```bash
# Adicionar o repositório do Helm chart
helm repo add titlis https://charts.titlis.io
helm repo update

# Instalar o operator
helm install titlis-operator titlis/titlis-operator \
  --namespace titlis-system \
  --create-namespace \
  --set titlis.apiKey=SUA_CHAVE_AQUI \
  --set titlis.apiHost=SEU_ENDPOINT_TITLIS
```

Substitua `SUA_CHAVE_AQUI` pela chave gerada no passo anterior e `SEU_ENDPOINT_TITLIS`
pelo endereço da plataforma fornecido pela sua equipe.

### Verificar se o operator está rodando

```bash
kubectl get pods -n titlis-system
# titlis-operator-xxxxx   1/1   Running   0   2m
```
...
- Os logs do pod: `kubectl logs -n titlis-system -l app=titlis-operator`
